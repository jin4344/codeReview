...
function showChat(type, user, message, subtxt, mid) {
    ...
    if (!subtxt) {
        $("#panel").append(
            '<div data-mid="' +
                ...
                '""><span class="name ' +
                nameclass +
                '"><b><a class="namelink" href="javascript:void(0)">' +
                user +
                '</a></b></span><span class="delete"><a href="javascript:void(0)">DELETE</a></span><span class="timestamp">' +
                getTime() +
                '</span><span class="msg">' +
                message +
                "</span></div>"
    ...
    if (settings.inline) {
        var m = message.match(/(https?|ftp):\/\/[^\s/$.?#].[^\s]*/gim);
...
$(document).ready(function () {
    ...
    $("#recognition").bind("change", function () {
        ...
        if (settings.recognition) $("#audio").show();
        else {
            $("#audio").hide();
        }
    });
